...
VM=102; WORK=/tmp/hv238-seerr; rm -rf "$WORK"; mkdir -p "$WORK"
...
cp /tmp/hv238-seerr/docker-compose.yml /opt/homelab/seerr/docker-compose.yml
cp /tmp/hv238-seerr/.env /opt/homelab/seerr/.env
...
rscp "$WORK" "$VM" "/tmp/"
rssh "$VM" "sudo /tmp/hv238-seerr/install.sh"
