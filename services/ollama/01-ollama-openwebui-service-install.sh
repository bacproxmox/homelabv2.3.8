...
cp /tmp/hv238-ollama/.env /opt/homelab/ollama/.env
...
ℹ️ v2.3.8 core install büyük modelleri otomatik indirmez.
Model indirmek için: Additionals menu > AI / Ollama model management
MSG
REMOTE
chmod +x "$WORK/install.sh"
rscp "$WORK" "$VM" "/tmp/"
rssh "$VM" "sudo /tmp/hv238-ollama/install.sh"
