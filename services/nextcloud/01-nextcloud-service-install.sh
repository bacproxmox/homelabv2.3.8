...
prepare_truenas_nextcloud_permissions() {
  ...
  tn_get "system/info" >/tmp/hv238-truenas-info.json || {
  ...
  tn_get "sharing/nfs" >/tmp/hv238-truenas-nfs.json || echo "[]" >/tmp/hv238-truenas-nfs.json
  nfs_id="$(python3 - <<'PY'
import json
from pathlib import Path
try:
    data=json.loads(Path('/tmp/hv238-truenas-nfs.json').read_text() or '[]')
except Exception:
    data=[]
for item in data:
...
PY
  ...
  payload='{"path":"/mnt/tank/nextcloud","comment":"Nextcloud user data NFS","enabled":true,"networks":["192.168.50.0/24"],"maproot_user":"root","maproot_group":"root","ro":false}'
  if [[ -n "$nfs_id" ]]; then
    echo "📡 TrueNAS NFS update: /mnt/tank/nextcloud maproot=root"
    tn_put "sharing/nfs/id/${nfs_id}" "$payload" >/dev/null || true
  else
    echo "📡 TrueNAS NFS create: /mnt/tank/nextcloud maproot=root"
...
if ! grep -qsE '^192\.168\.50\.101:/mnt/private/documents\s+/mnt/private-documents\s+' /etc/fstab; then
  echo '192.168.50.101:/mnt/private/documents /mnt/private-documents nfs defaults,_netdev,x-systemd.automount,nofail 0 0' >> /etc/fstab
fi
...
cp /tmp/hv238-nextcloud/docker-compose.yml /opt/homelab/nextcloud/docker-compose.yml
cp /tmp/hv238-nextcloud/.env /opt/homelab/nextcloud/.env
...
rscp "$WORK" "$VM" "/tmp/"
rssh "$VM" "sudo /tmp/hv238-nextcloud/install.sh"
