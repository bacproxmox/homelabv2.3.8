...
VM=106; WORK=/tmp/hv238-immich; rm -rf "$WORK"; mkdir -p "$WORK"
...
cp /tmp/hv238-immich/docker-compose.yml /opt/homelab/immich/docker-compose.yml
cp /tmp/hv238-immich/.env /opt/homelab/immich/.env
...
rscp "$WORK" "$VM" "/tmp/"
rssh "$VM" "sudo /tmp/hv238-immich/install.sh"
