...
echo
echo "🎬 Homelab v2.3.8 - Jellyfin Post-Setup Configurator"
echo

USERS_ENV="/root/homelab-secrets/users.env"
JELLYFIN_USERS_ENV="/root/homelab-secrets/jellyfin-users.env"
...
apt update
apt install -y sshpass curl jq openssl

mkdir -p /root/homelab-secrets

if [[ ! -f "$JELLYFIN_USERS_ENV" ]]; then
  DEFAULT_VIEWER_PASS="$(openssl rand -base64 18 | tr -d '=+/' | cut -c1-16)"

  cat > "$JELLYFIN_USERS_ENV" <<EOF
JELLYFIN_VIEWER_DEFAULT_PASS=$DEFAULT_VIEWER_PASS
EOF

  chmod 600 "$JELLYFIN_USERS_ENV"
fi

set -a
source "$JELLYFIN_USERS_ENV"
set +a

VIEWER_PASS="${JELLYFIN_VIEWER_DEFAULT_PASS:-ChangeMe123}"
...
for viewer in "\${VIEWERS[@]}"; do
  ...
  if [[ -z "\$EXISTING_ID" ]]; then
    ...
    USER_CREATE="\$(curl -sS -X POST \
      ...
      --data "{\"Name\":\"\$viewer\",\"Password\":\"\$VIEWER_PASS\"}" || true)"
...
printf '  - %s\n' "\${VIEWERS[@]}"
echo
echo "🔑 Varsayılan viewer şifresi:"
echo "  \$VIEWER_PASS"
echo
echo "⚠️ Kullanıcılar ilk login sonrası şifre değiştirmeli."
EOF
