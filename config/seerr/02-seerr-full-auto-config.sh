...
echo
echo "🎟️ Homelab v2.3.8 - Seerr Full Auto Configurator"
echo
...
for col, value in [("permissions", 1048575), ("userType", 2), ("isAdmin", 1)]:
    if col in cols:
        cur.execute(f'UPDATE "{user_table}" SET "{col}"=? WHERE email=? OR username=?', (value, SERVICE_EMAIL, SERVICE_USER))
conn.commit()
conn.close()
