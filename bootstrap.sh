...
REPO_URL="${REPO_URL:-https://github.com/bacproxmox/homelabv2.3.8.git}"
INSTALL_DIR="${INSTALL_DIR:-/root/homelabv2.3.8}"
BRANCH="${BRANCH:-main}"
BOOTSTRAP_MARKER="/root/.homelabv2.3.8-bootstrap-reboot-done"
export HOMELAB_VERSION="2.3.8"
...
safe_reboot_gate() {
  if need_safe_reboot; then
    if [[ ! -f "$BOOTSTRAP_MARKER" ]]; then
      ...
      echo "bash <(curl -fsSL https://raw.githubusercontent.com/bacproxmox/homelabv2.3.8/main/bootstrap.sh)"
      echo
      sleep 8
      reboot
    else
...
echo
echo "Starting Homelab v2.3.8 menu..."
