...
echo
echo "🔐 Homelab v2.3.8 - secrets/env bootstrap"
echo "Klasör: $SECRETS_DIR"
echo "Not: Gizli input yok. Yazdığın değerler terminalde görünür."
echo

if [[ ! -f "$SECRETS_DIR/global.env" ]]; then
  ask_text_default DOMAIN "Ana domain" "bacmastercloud.com"
  ask_text_default LAN_GW "Gateway" "192.168.50.1"
  ask_text_default LAN_DNS "DNS" "1.1.1.1"
  ask_text_default VM_STORAGE "Proxmox VM storage" "nvme-vm"
  {
    write_env_header
    write_env_line HOMELAB_VERSION "2.3.6"
