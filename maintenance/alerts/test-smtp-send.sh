...
test_one(){
  ...
  echo "========================================="
  echo "SMTP host : ${SMTP_HOST:-smtp.zoho.com}"
  ...
  SERVICE="$service" APP_PASS="$app_pass" SMTP_HOST="${SMTP_HOST:-smtp.zoho.com}" SMTP_PORT="${SMTP_PORT:-587}" SMTP_FROM="${SMTP_FROM:-admin@bacmastercloud.com}" SMTP_TEST_TO="${SMTP_TEST_TO:-admin@bacmastercloud.com}" python3 - <<'PYMAIL'
...
msg['From']=user
msg['To']=to
msg['Subject']=f'Homelab v2.3.8 SMTP test - {service}'
msg.set_content(f'Homelab v2.3.8 SMTP test başarılı. Service profile: {service}')
try:
