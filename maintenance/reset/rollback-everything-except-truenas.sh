...
KEEP_VMID="101"
DESTROY_VMIDS=(102 103 104 105 106 107)
...
Bu işlem VM102-VM107 arasındaki VM'leri durdurup silecek.
...
if ! qm status "$KEEP_VMID" >/dev/null 2>&1; then
  echo "⚠️ VM101 TrueNAS bulunamadı. Yine de VM102-107 rollback yapılabilir."
else
...
for ip in 192.168.50.102 192.168.50.103 192.168.50.104 192.168.50.105 192.168.50.106 192.168.50.107; do
